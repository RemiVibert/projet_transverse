...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, game):
        ...
        self.has_launched = False # Vérifie si le joueur a déjà été lancé
        

        self.pos:pygame.Vector2 = pygame.Vector2(0, 0) # Position du vaisseau
        ...
        self.rect_out_of_bounds = self.image_out_of_bounds.get_rect(center=self.pos)

    def update(self):

        if self.dragging:
            self.pos += (self.velocity)/3 * self.game.dt # Met un effet de ralentit quand on drag
        else :
        ...
        if self.has_launched: # La gravité et les collisions ne s'appliquent que si le vaisseau a été lancé
            ...
            self.pos:pygame.Vector2 = pygame.Vector2(self.pos) # Pour s'assurer que c'est un vecteur et non un tuple
            ...
            for planet in self.game.planets:
                ...
                if distance < total_radius: # Calculer la collison
                    if planet.type != "gazeuse":
                        self.game.game_over("crash", False) #Ne meurt pas en entrant en contact avec un gazeuse

                        direction = (self.pos - planet.pos).normalize()
                        self.pos = planet.pos + direction * (planet.radius + self.radius) # Déplacer le vaisseau à la périphérie de la planète

                        self.velocity = self.velocity.reflect(direction) # On reflète la vélocité pour simuler un rebond
                        self.velocity *= 0.8  # On réduit un peu la vitesse pour simuler la perte d'énergie
    ...
    def draw(self, screen, camera):
        ...
        # Affichage du nombre de collectibles récupérés
        font = pygame.font.Font(None, 36)
        text = font.render(f"{self.collected_collectibles}/{self.game.nb_collectibles}", True, (255, 255, 255))
